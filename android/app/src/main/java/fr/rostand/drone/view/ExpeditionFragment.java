...
public class ExpeditionFragment extends Fragment implements View.OnClickListener {
    ...
    private void initUI() {
        ...
        mSurfaceView.getHolder().addCallback(new SurfaceHolder.Callback() {
            ...
            public void surfaceCreated(SurfaceHolder holder) {
                // Method is called when surface is ready to draw
    ...
    /**
     * Draw waypoint list on screen
     */
    private void drawCoordList(List<Waypoint> waypointList) {
        Log.d(TAG, "onDraw");

        AsyncTask.execute(() -> {
            // If SurfaceHolder is ready to draw
            if (mSurfaceHolder != null && mSurfaceHolder.getSurface().isValid()) {
                ...
                // Draw rectangle that represents area to shoot
                for (int i = 0; i < 4; i++) {
